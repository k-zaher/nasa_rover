...
describe NasaRover::Plateau do
  ...
  it "should ignore commands that gets the rover
    out of the plateau's maximum bounds" do
    commands = 'MMMMM'
    ...
    expect(@current_rover.position.to_text).to eq('1 5 N')
  end
  it "should ignore commands that gets the rover
    out of the plateau's minimum bounds" do
    commands = 'LMM'
    commands.chars.each do |command|
      @current_rover.position = @control.execute!(
        command,
    ...
    expect(@current_rover.position.to_text).to eq('0 2 W')
  end
end
